        ...
        <div class="order_box">
            ...
            <div class="user_info js_copy_info"> <!--js_copy_info是购买人专用的，下面得复制购买人信息按钮点击,需要读取这个名字下面输入得内容-->
                ...
                <dl class="user_dl">
                    ...
                    <dd>
                        <input class="input js_yz js_goumai_name" type_name="text" type="text" placeholder="姓名" value="">
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            for(var i=0; i<data.length; i++) {
                nameList = nameList + '<label class="check"><input class="checkbox" type="checkbox" value="'+data[i].receiverId+'" id="passenger_' + data[i].certNo + '" onclick="checkUser(this)">' + data[i].receiverName
...
function checkBooking(){
    ...
    var adults = ${passengers.adultCount*quantity};
    var childs = ${passengers.childCount*quantity};
...
function cleanSameUser(receiverId,certNo,name,mobile){
    if($.trim(receiverId) || $.trim(certNo)){
        $(".border_t1_dotted").each(function(){
            if($.trim(receiverId) == $.trim($(this).find("input[name='receiverId']").val()) ||   $.trim(certNo) == $.trim($(this).find("input[type_name='shenfenzheng']").val())){
                var $youwan1_receiverId = $('.order_box').children('.user_info:eq(0)').children('dl:eq(0)').find('input:eq(1)').val();
                $(this).find("input").val("");
                ...
                //获取游玩人1的名字、电话，将购买人和游玩人1的名字和电话始终保持一致
                //如果清空的是第一个游玩人,才将购买人信息相应的删除
                if($youwan1_receiverId==receiverId){
                    var $youwan1_name = $('.order_box').children('.user_info:eq(0)').children('dl:eq(0)').find('input:eq(0)').val();
                    $('.js_goumai_name').val($youwan1_name);
                    var $youwan1_mobile = $('.order_box').children('.user_info:eq(0)').children('dl:eq(2)').find('input:eq(0)').val();
                    $('.js_textBig').val($youwan1_mobile);
                    ...
                    $(".js_copy_info").find(":hidden[name='receiverId']").val($youwan1_receiverId);
                    var $youwan1_email = $('.order_box').children('.user_info:eq(0)').children('dl:eq(5)').find('input:eq(0)').val();
                    $(".js_copy_info").find("input[type_name='email']").val($youwan1_email);
                }
                return false;
            }
       });
