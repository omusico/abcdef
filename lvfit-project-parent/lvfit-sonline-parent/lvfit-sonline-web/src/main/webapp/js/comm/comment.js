...
var Comment = {
		...
		newLoadPaginationOfComment:function(param){
			...
			$.ajax({
				...
				data:
				{
					requestStr:"http://dujia.lvmama.com/vst_front/comment/newPaginationOfComments?type="+param.type+
					"&currentPage="+(param.currentPage==undefined?1:param.currentPage)+
					"&totalCount="+(param.totalCount==undefined?"":param.totalCount)+
					"&placeId="+(param.placeId==undefined?"":param.placeId)+
					"&productId="+(param.productId==undefined?"":param.productId)+
					"&placeIdType="+(param.placeIdType==undefined?"":param.placeIdType)+
					"&isPicture="+(param.isPicture==undefined?"":param.isPicture)+
					"&isBest="+(param.isBest==undefined?"":param.isBest)+
					"&isPOI="+(param.isPOI==undefined?"":param.isPOI)+
					"&isELong="+(param.isELong==undefined?"":param.isELong)+
					"&productRef="+(param.productRef==undefined?"":param.productRef)+
					"&vstType="+(param.vstType==undefined?"":param.vstType)+
					"&isRef="+(param.isRef==undefined?"":param.isRef)
				},
				type : "POST",
				datatype : "json",
