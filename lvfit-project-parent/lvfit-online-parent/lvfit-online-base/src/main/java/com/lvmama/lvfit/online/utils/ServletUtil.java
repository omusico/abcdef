package com.lvmama.lvfit.online.utils;
...
import com.lvmama.lvfit.online.constants.UserConstant;
...
public class ServletUtil {
	...
	public static String getRequestHost(HttpServletRequest request) {
		...
		if (host==null) {
			return UserConstant.DEFAULT_LOCATION;
		}if (host.indexOf(".")!=-1) {
	 ...
	 * @param obj
	 */
	public static void putVstSession(String vstLvSessionId,HttpServletRequest request,String key, Object obj) {
		Map<String, Object> map = getSessionMap(vstLvSessionId);
		map.put("LAST_UPDATE_TIME", System.currentTimeMillis());
		map.put(key, obj);
		boolean result = MemcachedUtil.getInstance().set(vstLvSessionId, 1800, map,true);
		if(log.isDebugEnabled()){
			log.info("put object to memcached, session:" + vstLvSessionId+ " result: " + result);
		}
	}
	 ...
	 * @return
	 */
	public static Object getVstSession(String vstLvSessionId,String key) {
		try{
			Map<String, Object> map = getSessionMap(vstLvSessionId);
			Object obj = map.get("LAST_UPDATE_TIME");
			if (obj!=null) {
				...
				if (period>300000) {
					int ver = Integer.parseInt(map.get("ver").toString());
					map.put("ver", ++ver);
					MemcachedUtil.getInstance().replace(vstLvSessionId, 1800, map,true);
				}
			}
			return map.get(key);
		}catch(Exception e) {
	 ...
	 * @param key
	 */
	public static void removeVstSession(String vstLvSessionId,HttpServletRequest request,HttpServletResponse response, String key) {
		Map<String, Object> map = getSessionMap(vstLvSessionId);
		Object obj = map.get(key);
		if(log.isDebugEnabled()){
			log.debug("REMOVE SESSION : key: " + key + " value : " + obj + " from " + vstLvSessionId);
		}
		map.remove(key);
		int ver = Integer.parseInt(map.get("ver").toString());
		map.put("ver", ++ver);
		MemcachedUtil.getInstance().replace(vstLvSessionId, 1800, map,true);
		Object obj1 = MemcachedUtil.getInstance().get(vstLvSessionId,true);
	}
	
	
	public static String getLvSessionId(HttpServletRequest request, HttpServletResponse response) {
		if(request.getAttribute(UserConstant.LV_SESSION_ID)!=null){
			return request.getAttribute(UserConstant.LV_SESSION_ID).toString();
		}else {
			return getCookieValue(request,UserConstant.LV_SESSION_ID);
		}
	}
	...
	public static void initLvSessionId(HttpServletRequest request, HttpServletResponse response) {
		Cookie cookie = getCookie(request, UserConstant.LV_SESSION_ID);
		String lvsessionId = cookie==null?null:cookie.getValue();
		if (lvsessionId==null) {
			if(!StringUtil.isEmptyString(request.getParameter(UserConstant.LV_SESSION_ID))){
				lvsessionId = request.getParameter(UserConstant.LV_SESSION_ID);
			}else{
				lvsessionId = UUID.randomUUID().toString()+"_"+(int)((Math.random()+1)*10000000);
			}
			String host=request.getServerName();
			boolean use_lvmama_host=host.contains(".lvmama.com");
			addCookie(response, UserConstant.LV_SESSION_ID, lvsessionId,use_lvmama_host);
			request.setAttribute(UserConstant.LV_SESSION_ID, lvsessionId);
		}
	}
	
	public static void initLvSessionId(HttpServletRequest request, HttpServletResponse response,String userId){
		Cookie cookie = getCookie(request, UserConstant.LV_SESSION_ID);
		String lvsessionId = cookie==null?null:cookie.getValue();
		if (lvsessionId==null) {
			if(!StringUtil.isEmptyString(request.getParameter(UserConstant.LV_SESSION_ID))){
				lvsessionId = request.getParameter(UserConstant.LV_SESSION_ID)+"_"+userId;
			}else{
				lvsessionId = UUID.randomUUID().toString()+"_"+userId;
			}
			String host=request.getServerName();
			boolean use_lvmama_host=host.contains(".lvmama.com");
			addCookie(response, UserConstant.LV_SESSION_ID, lvsessionId,use_lvmama_host);
			...
			if(lvsessionId.indexOf("_") != -1){
				lvsessionId = lvsessionId.substring(0,lvsessionId.lastIndexOf("_"));
			}
			lvsessionId = lvsessionId+"_"+userId;
			String host=request.getServerName();
			boolean use_lvmama_host=host.contains(".lvmama.com");
			addCookie(response, UserConstant.LV_SESSION_ID, lvsessionId,use_lvmama_host);
			request.setAttribute(UserConstant.LV_SESSION_ID, lvsessionId);
	...
	/*public static boolean isMobileLogin(HttpServletRequest request){
		...
		if(UserConstant.LOGIN_TYPE.MOBILE.name().equals(loginTypeName)){
			return true;
		}
		if(UserConstant.LOGIN_TYPE.MOBILE.name().equals(request.getParameter("loginType"))){
			return true;
		}
		return false;
...
/*	public static boolean isMobileDeviceLogin(HttpServletRequest request){
	...
	*/
	...
	public static boolean isWapLogin(HttpServletRequest request){
		...
		if(UserConstant.LOGIN_TYPE.HTML5.name().equals(loginTypeName)){
			return true;
		}
		if(UserConstant.LOGIN_TYPE.HTML5.name().equals(request.getParameter("loginType"))){
			return true;
		}
		return false;
	...
	*/
	public static void main(String[] args) {
		...
		map1.put("user", new FitMemUserDto());
