...
public class PageConfig<T> implements Serializable {
	...
	private static final long serialVersionUID = -5874657778068540604L;
	...
	public List<T> getItems() {
		if (items == null) {
			...
			if (allItems.size() > 0) {
				...
				int end = this.getCurrentRowNum() < this.allItems.size() ?  this.getCurrentRowNum() :this.allItems.size() ;
				for (; i < end; i++) {
