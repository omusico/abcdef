...
public class StringUtil { 
	 ...
	 * @return
	 */
	public static String getSortSring(String str,int defaultValue)
	{
		 if(StringUtils.isNotBlank(str)){
	      	    byte[] remarkBytes = str.getBytes();
	            if (remarkBytes.length > defaultValue) {
	                byte[] newBytes = Arrays.copyOfRange(remarkBytes, 0, defaultValue);
	                str = new String(newBytes);
	            }
	      }
		  return str;
	}
