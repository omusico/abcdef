...
import com.lvmama.lvf.common.dto.status.OrderAuditType;
...
public enum OrderAuditStatus implements GeneratorOp,ProcessOp {
		...
		public List<Op> generatorBackOp(OpContext opContext){
			...
			switch (opContext.getOpType()) {
				...
				case RTVT:
					...
					opList.add(OpCommandBack.APPLY_SUPP_RTVT);
	...
	public void processOp(StatusContext statusContext) {
		...
		if(vaild(statusContext)){
			statusContext.getOrder().getFlightOrderStatus().setOrderAuditStatus(this);
		}
	};
}
