 ...
 * @date 2015-3-12
 */
public enum OrderTicketStatus implements GeneratorOp,ProcessOp {
	...
	RTVT_DONE("已退票",TicketStatusType.RTVT){
		@Override
		public boolean vaild(StatusContext statusContext) {
			FlightOrderDto order = statusContext.getOrder();
			if (OrderType.RTVT.equals(order.getOrderType())) {
...
//		}
	...
	public void processOp(StatusContext statusContext) {
		...
		if(vaild(statusContext)){
			statusContext.getOrder().getFlightOrderStatus().setOrderTicketStatus(this);
		}
	}
