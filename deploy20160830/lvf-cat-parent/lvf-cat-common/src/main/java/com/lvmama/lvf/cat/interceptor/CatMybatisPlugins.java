...
public class CatMybatisPlugins implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        Transaction t = dbCatHandler(invocation);
	...
	private Transaction dbCatHandler(Invocation invocation) {
		...
		try {
            ...
            t = Cat.newTransaction(CatComConstants.SQL,class_method);
            if(dataSource instanceof BasicDataSource){
                BasicDataSource data= (BasicDataSource)dataSource;
                String url = data.getUrl();
                Cat.logEvent(CatComConstants.SQL_DATABASE, url);
            }
            Cat.logEvent(CatComConstants.SQL_STATEMENT, sql.substring(0, sql.indexOf(" ")), Message.SUCCESS, sql);
		} catch (Exception e) {
            Cat.logError(e);
		}
		return t;
